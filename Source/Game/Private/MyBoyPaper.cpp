...
/// 
	...
	Collision->OnComponentBeginOverlap.AddDynamic(this, &AMyBoyPaper::OnOverlapBegin);
	Collision->OnComponentEndOverlap.AddDynamic(this, &AMyBoyPaper::OnOverlapEnd);
...
// Called every frame
void AMyBoyPaper::Tick(float DeltaTime)
{
...
void AMyBoyPaper::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	Super::SetupPlayerInputComponent(PlayerInputComponent);
	
	PlayerInputComponent->BindAxis(TEXT("MoveVert"), this, &AMyBoyPaper::MoveVert);
	PlayerInputComponent->BindAxis(TEXT("MoveHoriz"), this, &AMyBoyPaper::MoveHoriz);
	PlayerInputComponent->BindAxis(TEXT("CamZoom"), this, &AMyBoyPaper::Zoom);
	
	PlayerInputComponent->BindAction("Jump", IE_Pressed, this, &AMyBoyPaper::CheckJump);
	PlayerInputComponent->BindAction("Jump", IE_Released, this, &AMyBoyPaper::CheckJump);

	PlayerInputComponent->BindAction(TEXT("Action|Take"), IE_Pressed, this, &AMyBoyPaper::ChangeTower);
	PlayerInputComponent->BindAction(TEXT("Action|Place"), IE_Pressed, this, &AMyBoyPaper::PlaceTower);
	
}

void AMyBoyPaper::CheckJump()
{
	if(Jumping)
	{
...
void AMyBoyPaper::MoveVert(float AxisValue)
{
	if(AxisValue == 0.f){
		bMoveVert = false;
		return;
...
void AMyBoyPaper::MoveHoriz(float AxisValue)
{
	if(AxisValue == 0.f){
		bMoveHoriz = false;
		return;
...
void AMyBoyPaper::Zoom(float AxisValue)
{
	if(AxisValue == 0.f)
	{
		return;
...
void AMyBoyPaper::SpawnTower(float DeltaTime)
{
	if(TowerMember != nullptr)
	{
		TowerMemberLocation.X = round((TowerMemberLocationShift.X + this->GetActorLocation().X) / TILE_SIDE_LEN) * TILE_SIDE_LEN;
...
void AMyBoyPaper::OnOverlapBegin(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp,
...
void AMyBoyPaper::OnOverlapEnd(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp,
